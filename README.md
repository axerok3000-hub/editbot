...
«🗑 Мои удалённые», «✨ Эффекты», «🎮 Игры» (последние две пока заглушки).
Также работают текстовые команды `/exclude <chat_id>` и
`/include <chat_id>` для управления списком чатов, где эффект отключён.
Реагирует на всё это только `OWNER_ID` — все остальные отправители молча
игнорируются.
